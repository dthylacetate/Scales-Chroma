...
这一轮目标不是继续堆新视觉层，而是解决真实使用里的三个理解问题：

- 右侧说明有些过于形式化，对不懂乐理的人不够友好。
- 中间舞台左上角还有很多英文角标，不利于直接理解。
- 左侧元素库没有按使用者心智分组，也缺少自定义和弦进行。
...
- 和弦进行区新增自定义输入：
  ...
  - 支持 `V-I` 这类基础罗马数字级数
- 前端会根据当前已选元素做轻量相容性判断：
  - 明亮调式或 `Maj7` 遇到 `ii-V-I / V-I` 会提示“很相容”
  - 暗色调式加 `Dim7` 或很多小级数会提示“偏紧张”
  - `Dominant7` 如果没有对应 V 级，会提示“可实验”
- 舞台左上角角标改成中文，例如：
  - `大型装置`
  - `导演节奏`
  - `空气质感`
  ...
  - `模块痕迹`
  - `舞台席位`
- 右侧核心解释改成更白话的中文，例如把 `Valence / Arousal / Luminosity / Grit` 改成 `明暗情绪 / 兴奋程度 / 发光空间 / 颗粒粗糙`。

对应测试：

- UI 测试：
  - 验证元素库按四个大类展示。
  - 验证 `ii-V-I` 自定义进行会给出“很相容”和“有明显回家感觉”的提示。
  - 验证自定义进行可以加入编排轨道。
  - 验证右侧 `舞台解读`、`听感性格`、`组合相容性` 等中文面板存在。
- 拖拽测试：
  - 同步更新按钮可访问名称，确认分类后的模块仍可拖到轨道和舞台。
- 烟测脚本：
  - 同步更新真实浏览器脚本，让它检查中文按钮、中文角标和中文情绪读数。

验证结果：

- 前端：`105 passed`
- 前端 build：通过
- 真实浏览器烟测：通过，确认 `舞台解读`、`明暗情绪`、中文分类按钮和核心视觉链路都能在单端口服务中工作。

## 最新一轮：自定义进行知识库与练习主题模糊匹配

这一轮继续围绕“用户真实会怎么输入”来补能力。两个重点都不是做复杂后台，而是让系统更懂普通用户的表达：

- 用户可能不会写完整罗马数字，只会输入 `251`、`1251`、`4536`。
- 用户练习主题也可能写中文口语，例如“下拨”“融合连奏”，系统不能只认英文标签。

实现方式：

- 前端自定义和弦进行新增数字级数解析：
  - `251` 会转成 `ii-V-I`。
  - `1251` 会转成 `I-ii-V-I`，并识别其中包含经典 `II-V-I`。
  - `4536` 会转成 `IV-V-iii-vi`，按常见华语/日系流行进行解释。
- 前端新增一个小型进行模式库：
  - `II-V-I`
  - `I-II-V-I`
  - `I-V-vi-IV`
...
- 自定义进行说明拆成两层：
  - 先讲整体命中的常见进行。
  - 再讲相邻两个和弦之间的连接，比如 `I -> ii` 是从稳定处往外走，单看有点突然，但接上 `V-I` 就合理。
- 后端新增 `topic_matching` 服务，把练习主题和技能路线/解锁关键词做统一模糊匹配。
- Metal / Fusion / Jazz / Neo Soul / Pentatonic 都加入常见中文和英文别名，避免用户必须输入非常标准的英文标签。

对应测试：

- 前端 UI 测试：
  - 输入 `1251` 后，页面必须能识别 `II-V-I` 片段。
  - 输入 `4536` 后，页面必须能识别 `IV-V-iii-vi`，并出现“华语/日系流行”的说明。
  - 页面必须显示相邻连接解释，例如 `I 到 ii 是从稳定处往外走`。
- 后端集成测试：
  - 练习 `下拨速度练习` 后，Metal 路线的技能树和视觉解锁应被推动。
  - 练习 `融合连奏句子` 后，Fusion 路线的技能树和视觉解锁应被推动。

验证结果：

- 后端：`99 passed, 1 warning`
- 前端：`106 passed`
- 前端 build：通过
- 仍有 1 个 React `act(...)` 测试提示，来自旧的组合保存测试，不影响当前功能。
- 后端 warning 仍是 FastAPI/Starlette TestClient 依赖层的弃用提示，不是业务代码错误。

## 最新一轮：技能树与基础乐理独立子版块

这一轮攻克的是信息架构问题。之前技能树和乐理解释都被塞在沙盘主界面里，用户如果刚进入项目，很容易不知道“我该学什么、练什么、为什么舞台会这样变”。所以这次把项目从单页沙盘扩成三个可切换子版块：

- `视觉沙盘`：保留原来的拖拽、演出、练习记录和组合保存。
- `技能树`：单独解释成长路线、解锁方式、练习主题和舞台变化。
- `基础乐理`：做成新手可读的入门页面，覆盖音律、调式、和弦、进行等基础。

实现方式：

- `frontend/src/App.tsx` 新增轻量内部导航，不引入额外路由库，避免上线前增加依赖和复杂度。
- 新增 `frontend/src/pages/SkillTreePage.tsx`：
  - 登录态下读取 `/skill-tree` 与 `/unlocked-effects`。
  - 非登录或开发模式下展示 demo 技能树，保证页面仍可阅读。
  - 对 Metal / Jazz / Fusion / Neo Soul 分别说明练习主题、解锁能力和舞台变化。
- 新增 `frontend/src/pages/MusicTheoryGuide.tsx`：
  - 解释音高、半音、音程。
  - 解释十二平均律、纯律和吉他品格为什么对应半音。
  - 解释 Major / Minor / Pentatonic / Harmonic Minor / Melodic Minor。
  - 解释 Ionian / Dorian / Phrygian / Lydian / Mixolydian。
  - 解释 Major / Minor / Maj7 / Min7 / Dominant7 / Dim7 / Aug。
  - 解释 I-V-vi-IV、ii-V-I、IV-V-iii-vi 等常见进行。
  - 解释张力与解决如何映射到颜色、几何、粒子和运动。

对应测试：

- 新增 `LearningPages` UI 测试：
  - 验证顶部导航能从沙盘切到技能树，再切到基础乐理。
  - 验证基础乐理页确实包含音高/半音、音律、调式、和弦、和弦进行等核心内容。
  - 验证技能树页会读取登录态接口，并展示解锁路线说明。
- 更新 `AppRuntimeConfig` 测试：
  - 顶部导航和沙盘内用户显示会同时出现用户名，因此测试改成允许多个可见用户名，避免因为新导航误判失败。
- 浏览器实测：
  - 打开本地 Vite 页面。
  - 点击 `技能树`，确认“练习会改变你的舞台能力”和“怎么解锁？”可见。
  - 点击 `基础乐理`，确认“从‘会听见颜色’开始理解乐理”、调式与和弦进行内容可见。
