...
describe("stage voiceprints", () => {
  it("maps backend voiceprint labels into readable stage descriptors", () => {
    const voiceprints = getStageVoiceprints(baseVisual);

    expect(voiceprints).toHaveLength(3);
    expect(voiceprints[0]?.label).toBe("Sky Fan");
    expect(voiceprints[1]?.label).toBe("Velvet Halo");
    expect(voiceprints[2]?.label).toBe("Cadence Stairs");
  });

  it("filters out unknown voiceprints", () => {
    const voiceprints = getStageVoiceprints({
      ...baseVisual,
      voiceprints: ["Sky Fan", "Unknown Voice"]
    });

    expect(voiceprints).toHaveLength(1);
    expect(voiceprints[0]?.label).toBe("Sky Fan");
  });
});
