...
- 后端：76 passed，1 个 FastAPI/Starlette 测试客户端弃用 warning。
- 前端：46 passed，仍有 1 个 React `act(...)` 测试提示，不影响当前功能。
- 前端 build：通过。
- 浏览器烟测：已验证最新服务下的登录会话恢复、Growth 解锁后 `Unlocked Effects` 与 `Growth` 面板刷新，以及舞台签名从默认态切换到 `Velvet Glow`。
