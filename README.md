...
- 这一轮又把空气本身拆成了 `Stage Climate`。现在不同路线连空气介质都不同，会出现 `Solar Haze / Velvet Mist / Ember Ash / Choir Dust / Prism Fog / Tide Vapor / Neon Smog / Sanctum Smoke` 这些完全不同的环境层。
- 这一轮又把“拖拽顺序”正式接进了视觉引擎。现在同一组模块换个排列，也会触发不同的 `Phrase Trajectory`，例如 `Lift Arc / Velvet Drift / Forge Drop / Prism Climb / Runway Drive / Shadow Sink`，舞台会沿着不同主弧线和推进路径长出来。
- 这一轮继续把顺序细化成 `Phrase Hooks`。也就是说，不只是整句走向不同，**相邻模块之间怎么接**也会变成可见桥接动作，例如 `Skyline Rise / Cadence Sweep / Velvet Link / Runway Spark / Collapse Gate / Ritual Notch / Prism Ladder`，舞台会在局部节点长出不同的过桥手势。
- 这一轮继续把 Growth 和顺序系统真正缝到一起，新增 `Phrase Variation`。现在 Growth 不只是盖一层风格滤镜，而是会继续改写同一条句法轨迹，例如 `Choir Step / Silk Orbit / Hammer Fall / Phase Spiral / Spark Chase`，让“同一组顺序”在不同成长路线下也会长成不同推进人格。
- 这一轮继续把“每个模块都得看得见”做成了 `Element Voiceprints`。现在每块乐理积木都会留下自己的舞台痕迹，例如 `Sky Fan / Velvet Halo / Cadence Stairs / Voltage Spear / Fracture Crown`，组合越多，舞台上就会有越明确的分层参与感。
- 这一轮又把每块模块的“空间职责”拆成了 `Element Roles`。现在模块不仅会留下痕迹，还会占据不同舞台席位，例如 `Sky Lens / Halo Core / Cadence Rail / Voltage Core`，并用连接线把这些席位串成空间结构。
- 这一轮把沙盘左侧元素库重分为“音阶 / 调式 / 和弦类型 / 和弦进行”，不再把所有乐理积木混在一个长列表里。
- 和弦进行区新增“自定义进行”，支持输入 `I-vi-IV-V`、`ii-V-I`，也支持 `251`、`1251`、`4536` 这类数字级数写法。系统会识别其中的经典片段，并把整体进行和相邻两个和弦的连接关系用白话解释出来。
- 顶部已新增三个子版块入口：`视觉沙盘`、`技能树`、`基础乐理`。技能树和乐理学习不再只挤在沙盘侧栏里，可以跳转到独立页面阅读。
- `基础乐理` 页面已经覆盖音高/半音/音程、十二平均律、音阶、调式、和弦、和弦进行、张力与解决，目标是让新手读完后能开始看懂常见乐理教材。
- `技能树` 页面会单独解释 Metal / Jazz / Fusion / Neo Soul 的练习主题、解锁视觉能力和舞台变化。
- 中间舞台左上角角标已经改成中文，例如 `大型装置 / 导演节奏 / 空气质感 / 模块痕迹 / 舞台席位`，不用再靠英文参数猜含义。
- FastAPI 后端、SQLAlchemy 模型、SQLite 开发数据库。
- 最基础认证、登录注册、Bearer Token 会话恢复、用户数据隔离。
- EXP、连续签到、等级、技能树、年度热力图。
- 练习记录创建、历史读取、主题/日期筛选。
- 练习主题已加入中文/英文模糊匹配：例如“下拨”“金属 riff”会归入 Metal，“融合”“连奏”会归入 Fusion，用于技能树增长和视觉解锁。
- 用户永久解锁视觉特效，并明显改变沙盘舞台的几何重心、波纹、光束、粒子和氛围签名。
- 前端右侧已提供可读的 `Active Bonuses` 说明和中文特效解释，方便理解当前舞台为什么会这样变化。
- 前端右侧新增并继续优化 `舞台解读`，会直接把当前舞台的情绪、空间、运动和主要来源翻译成更白话的中文。
...
- 后端：99 passed，1 个 FastAPI/Starlette 测试客户端弃用 warning。
- 前端：109 passed，仍有 1 个 React `act(...)` 测试提示，不影响当前功能。
- 前端 build：通过。
...
- 最新这一轮前后端验证还确认 `Phrase Variation` 会随着 `Growth Imprint + Phrase Trajectory` 的组合一起出现，并同时出现在后端 `/sandbox/render`、左上角 `Variation: ...`、右侧 `Phrase Variation` 面板和 Canvas 的第二层句法改写层里。
- 最新这一轮前后端验证还确认 `Element Voiceprints` 会随着具体乐理模块一起出现，并同时出现在后端 `/sandbox/render`、左上角 `Voices: ...`、右侧 `Element Voiceprints` 面板和 Canvas 的独立模块痕迹层里。
- 最新这一轮前后端验证还确认 `Element Roles` 会随着具体乐理模块一起出现，并同时出现在后端 `/sandbox/render`、左上角 `Roles: ...`、右侧 `Element Roles` 面板和 Canvas 的舞台席位连接层里。
- 最新这一轮真实浏览器烟测还确认：
...
- 最新这一轮真实浏览器烟测又继续确认：
  - 在 `Growth Lens Preview` 切到 `Jazz` 后，`Lydian -> Maj7 -> II-V-I` 会真实出现 `Phrase Variation` 与 `Choir Step`
  - 页面左上角会同时出现 `Variation: Choir Step`
  - 在同一条预览链路里，`Valence` 会从明亮场景的 `1.00` 降到暗色场景的 `0.50`
- 最新这一轮真实浏览器烟测继续确认：
  - `Lydian -> Maj7 -> II-V-I` 会真实出现 `Element Voiceprints`
  - 页面里能直接读到 `Sky Fan` 与 `Cadence Stairs`
  - 左上角会出现 `Voices: 3`
- 最新这一轮真实浏览器烟测继续确认：
  - `Lydian -> Maj7 -> II-V-I` 会真实出现 `Element Roles`
  - 页面里能直接读到 `Sky Lens / Halo Core / Cadence Rail`
  - 左上角会出现 `Roles: 3`
- 最新这一轮真实浏览器烟测继续确认：
  - 页面中文角标、中文情绪读数和 `舞台解读` 都能在单端口服务中真实出现
  - 左侧分类后的 `调式 / 和弦类型 / 和弦进行` 按钮可以继续拖拽到舞台
  - 烟测读数已经从 `Valence` 更新为中文 `明暗情绪`
- 最新这一轮自动化验证还确认：
  - 自定义进行能识别 `1251` 里的 `II-V-I` 片段，以及 `4536` 里的 `IV-V-iii-vi` 流行进行。
  - 练习主题能模糊匹配中文关键词，例如“下拨”触发 Metal 路线，“融合”触发 Fusion 路线。
- 最新这一轮前端验证还确认：
  - 顶部导航可以在 `视觉沙盘 / 技能树 / 基础乐理` 之间切换。
  - `技能树` 独立页面能读取 `/skill-tree` 和 `/unlocked-effects`，并展示解锁路线说明。
  - `基础乐理` 独立页面包含调式、音阶、和弦、和弦进行等新手学习内容。
  - 本地浏览器实测确认三页导航和关键内容可见。
